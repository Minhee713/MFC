...
BEGIN_MESSAGE_MAP(CSerialPortDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_SEND, &CSerialPortDlg::OnBnClickedButtonSend)
...
BOOL CSerialPortDlg::OnInitDialog()
{
	...
	m_combo_port.SetCurSel(2);
	

	hCommWnd = this->m_hWnd;

	cComThread.OpenPort(_T("COM4"), CBR_9600, 8, ONESTOPBIT, NOPARITY); // Port 번호와 Baud Rate은 환경에 맞게 변경 가능

	if (cComThread.m_bConnected == TRUE) {
...
LRESULT  CSerialPortDlg::OnReceiveMessage(WPARAM wParam, LPARAM lParam)
{
	BYTE buffer;
	CString message;
	CString rstr;
	...
	length = cComThread.m_QueueRead.GetSize();
	m_edit_receive.GetWindowTextW(message);

	for (int i = 0; i < length; i++) {
		cComThread.m_QueueRead.GetByte(&buffer);
		message += buffer;
	}

	m_edit_receive.SetWindowTextW(message);
...
void CSerialPortDlg::OnBnClickedButtonSend()
{
	...
	buffer = new BYTE[str.GetLength() + 1];
	strcpy((char*)buffer, CT2A(str));



	cComThread.WriteComm(buffer, m_edit_send.GetWindowTextLengthW());
